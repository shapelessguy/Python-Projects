...
SYSTEM_PROMPT: str = f'''
You are an agent that proposes sentences in {NATIVE_LANGUAGE} and the user has to translate them in {TARGET_LANGUAGE}.
...
class TranslateSentenceFromNative(Node):
    system_prompt = SYSTEM_PROMPT
    accept_kw = "translate_sentence_from_native"
    description = "This agent is used to create exercises regarding sentence translations. " \
    f"The virtual assistant proposes a sentence in {NATIVE_LANGUAGE} and the user has to translate into {TARGET_LANGUAGE}."
    ...
    def process(self, input: Message):
        ...
        return Message(output_text, intent=TranslateSentenceFromNative, from_="assistant", history=input.history)
